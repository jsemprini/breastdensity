...
DID_NB2_mean <- function(can_type,...) {
  ...
  DID_est =  mean(subset(ms, R_col==R_sample & ms$year>R_year & ms$year<=A_year & ms$col_code==CAN_type & bd_law==1)[["up"]]) - 
    mean(subset(ms, R_col==R_sample & ms$year<R_year & ms$year>=B_year & ms$col_code==CAN_type & bd_law==1)[["up"]])
  - mean(subset(ms, R_col==R_sample & ms$year>R_year & ms$year<=A_year & ms$col_code==CAN_type & bd_law==0)[["up"]]) + 
    mean(subset(ms, R_col==R_sample & ms$year<R_year & ms$year>=B_year & ms$col_code==CAN_type & bd_law==0)[["up"]])
...
results1 <- do.call(rbind, replicate(1000, DID_NB2_mean(1) , simplify=FALSE ))
